...
def profile_edit(request, id):
    ...
    if request.method == 'POST':
        ...
        if request.user != user:
            return HttpResponse("未登录，你没有权限修改此用户信息。")
        ...
        if profile_form.is_valid():
            ...
            profile.save()
            if 'mydcim' in request.FILES:
                profile.mydcim = profile_cd["mydcim"]
            profile.save()
        ...
        context = {'profile_form': profile_form, 'profile': profile, 'user': user,'imgs': imgs}
